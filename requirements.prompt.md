...
**5. how to specify binding option**

In DataExplorerDialog api Result usually is an array of object, and we select one or more binding data element of api result in VisualDataPreview, so you must detect selected component instance schema or judge if it is an array, binding with all selected result, then component will show as repeated list for each binding an element, or if selected component instance is not array, only bind one element of from first selected element.

So you should save selected data info to help which data selected in for dynamically resolve.
useDataBinding function has no these info, so don't resolve any right data.

As normal, api datasource is an array, if binding one element, selected element identified by id in data. if it is an array, just binding all selected element.

But api result maybe changed, can you give me some advice and plan to specify how to identify. maybe on the api datasource define dialog or DataSourceDialog, specify binding option by selected element id or by selected index etc.

You should check \_\_schema to detect if binding an multiple element if schema is array, only one if schema is an object.

**6. Ignore mapping fields in props**

If props's field set ignore to mapping in API DataExplorerDialog, these fields not use mapping and data binding, so you need save these info for each field, for ignore API binding and mapping, don't clear original setting values in props, DataTab use existed setting values

**7. Add null transform**

For null or undefined value when binding, transform it to blank if it is string

**8. Add background or marker for binding field on DataTab form editor**
On DataTab for binding field and ignore field, can not see different, because binding field is not editable.

**9. Stackpage as lib used in next.js host app, how to add next.js app's api as datasource**
